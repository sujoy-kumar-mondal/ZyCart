...
const ForgotPassword = () => {
  ...
  useEffect(() => {
    document.title = "Merchant Password Recovery | ZyCart";
  }, []);
  ...
  const handleSendOtp = async (e) => {
    ...
    if (!email || !email.includes("@")) {
      return toast.error("Please enter a valid merchant email address.");
    }

    setLoading(true);
    try {
      const res = await axios.post("/auth/seller/send-reset-otp", { email });
      if (res.data.success) {
        toast.success("Merchant verification OTP sent to your email!");
        setStep(2);
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("/auth/seller/verify-reset-otp", {
        email,
        otp,
      ...
      if (res.data.success) {
        toast.success("Merchant password reset successful! Please log in.");
        navigate("/login");
      }
    } catch (error) {
          ...
          <div className="hidden lg:flex lg:col-span-5 bg-gradient-to-br from-[#0F172A] via-[#111C2E] to-[#062822] p-8 sm:p-10 flex-col justify-between relative border-r border-slate-800/70 overflow-hidden">
            ...
            <div className="space-y-6 relative z-10">
              <Link to="/login" className="inline-flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-white transition group">
                <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> Back to Merchant Sign In
              </Link>
              ...
              <div className="space-y-3">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-xs font-semibold text-emerald-300">
                  <Store className="w-3.5 h-3.5 text-emerald-400" /> Merchant Verification
                </div>
                ...
                <p className="text-xs text-slate-400 font-medium leading-relaxed">
                  Reset your password securely via OTP authentication to resume storefront operations and order fulfillment.
                </p>
              </div>

              {/* Step Flow Preview */}
              <div className="space-y-3 pt-2">
                <div className={`p-3.5 rounded-2xl border transition ${step === 1 ? "bg-emerald-500/15 border-emerald-500/40 text-white" : "bg-slate-800/40 border-slate-700/40 text-slate-400"}`}>
                  <div className="flex items-center gap-3">
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step === 1 ? "bg-emerald-500 text-white" : "bg-slate-700 text-slate-300"}`}>
                      1
                    </span>
                    <div>
                      <h4 className="text-xs font-bold text-white">Merchant Email Check</h4>
                      <p className="text-[11px] text-slate-400">Receive 2FA recovery code on registered address.</p>
                    </div>
                  </div>
                </div>

                <div className={`p-3.5 rounded-2xl border transition ${step === 2 ? "bg-emerald-500/15 border-emerald-500/40 text-white" : "bg-slate-800/40 border-slate-700/40 text-slate-400"}`}>
                  <div className="flex items-center gap-3">
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step === 2 ? "bg-emerald-500 text-white" : "bg-slate-700 text-slate-300"}`}>
                      2
                    </span>
                    <div>
                      <h4 className="text-xs font-bold text-white">Validate &amp; Set New Password</h4>
                      <p className="text-[11px] text-slate-400">Establish new commercial credentials.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="relative z-10 pt-6 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-400">
              <span>Merchant Network Support</span>
              <span className="font-semibold text-emerald-400">24/7 Monitored</span>
            </div>
          </div>
            ...
            <div className="max-w-md mx-auto w-full space-y-6">
              ...
              <div className="space-y-1 text-center lg:text-left">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-extrabold uppercase tracking-widest text-emerald-400 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                    Step {step} of 2
                  </span>
                  {step === 2 && (
                    <button
                ...
                <p className="text-xs text-slate-400 font-medium">
                  {step === 1
                    ? "Enter your registered merchant email address to receive a recovery code."
              ...
              {step === 1 ? (
                ...
                >
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-300">Registered Merchant Email</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-3 rounded-2xl bg-slate-800/60 border border-slate-700/80 text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
                        required
                        autoFocus
                      />
                    </div>
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white text-sm font-black shadow-lg shadow-emerald-500/25 transition-all transform active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer mt-2"
                  >
                    {loading ? (
                      <>
                  ...
                  <div className="pt-2 text-center">
                    <Link to="/login" className="text-xs font-bold text-slate-400 hover:text-white transition">
                      Remembered your password? <span className="text-emerald-400">Sign In</span>
                    </Link>
                  </div>
                </motion.form>
              ) : (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between gap-2" onPaste={handleOtpPaste}>
                      {otpValues.map((digit, index) => (
                        <input
                          ...
                          className="w-12 h-12 text-center text-lg font-black bg-slate-800/80 border border-slate-700/80 rounded-2xl text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition shadow-inner"
                          required
                        />
                      ))}
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-1">
                      ...
                      <button
                        ...
                        className="text-[11px] font-bold text-emerald-400 hover:text-emerald-300 disabled:text-slate-500 disabled:cursor-not-allowed transition cursor-pointer"
                      >
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        ...
                        <span>Reset Merchant Password</span>
                      </>
                    )}
                  </button>
                </motion.form>
