...
const Login = () => {
  ...
  const [form, setForm] = useState({
    email: localStorage.getItem("zycart_saved_admin_email") || "",
    password: "",
  });
  const [rememberMe, setRememberMe] = useState(
    Boolean(localStorage.getItem("zycart_saved_admin_email"))
  );
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    if (rememberMe) {
      localStorage.setItem("zycart_saved_admin_email", form.email);
    } else {
      localStorage.removeItem("zycart_saved_admin_email");
    }
    setLoading(true);
